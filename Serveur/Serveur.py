# -*- coding: utf-8 -*-
"""
Created on Fri Oct  9 08:47:48 2020
...
class ThreadClient(Thread):
    ...
    def run(self):
        ...
        while ((testFinServeur == False) and ( self.testFinClient == False )) :
            try:
                tailleImage = self.connexion.recv(8)#On récupère les 8 premier octets      
                tailleImage = int(tailleImage.decode())#On convertit la taille de l'image en entier (en octets)                
                print("tailleImage",tailleImage)
                
                contenuTelecharge = 0#Contenu téléchargé en octets
                
                fichierImage = open("download.jpg","wb") #Le fichier qui va contenir l'image
                ...
                while contenuTelecharge < tailleImage:
                    ...
                    contenuRecu = self.connexion.recv(1024)
                    #On ajoute la taille du contenu reçu au contenu téléchargé
                    contenuTelecharge += len(contenuRecu)
                    print("contenuRecu", type(contenuRecu))

                    
                    print("contenuTelecharge",tailleImage-contenuTelecharge, type(contenuRecu))
                    #On enregistre dans le fichier
                    if contenuTelecharge <= tailleImage:
                        print("ici")
                        fichierImage.write(contenuRecu)
                        #fichierImage.write(base64.decode(contenuRecu).decode('utf-8'))
                    else:
                        print("la")
                        dataSizeToSave=1024-(contenuTelecharge-tailleImage)
                        print(dataSizeToSave)
    ...
    def run(self):
        ...
        while testFinServeur==False:
            if(reconnaissance):
                reconnaissance=False
                try:
                    listeClient["Pepper"].sendall((reponse).encode())
                    print("ok")
                except:
                    print("Erreur Envoie")
        ...
        while(True):
            if(imgRecu==True):
                ...
                try:
                    imgRecu=False
                    resize("download")
                    reponse=calculRessemblance("download")
                    reconnaissance=True
                except Exception as inst:
                    print("err analyse")
                    print(inst)
...
def resize(image):                
    ...
    imageName = str(image)+".jpg" #Définition du nom de l'image à acquérir
...
def calculRessemblance(image):
    ...
    [X,y]=read_images("E:\\Cours\\S9\\PROJ942\\base_visage")
